...
export const pdfViewerAppRoutes: Object[] = [
    ...
    // tslint:disable-next-line:max-line-length
    { path: ':theme/pdfviewer/redaction', component: RedactionComponent , name: 'Redaction',type: 'update', order: '09', category: 'Editor', description: 'This PDF viewer sample facilitates the permanent removal of sensitive or confidential data from PDF files.'},
    // tslint:disable-next-line:max-line-length
    ...
    // tslint:disable-next-line:max-line-length
    { path: ':theme/pdfviewer/esigning-form-designer', component: ESigningFormDesignerComponent , name: 'eSigning Form Designer', order: '12', type: 'update', category: 'PDF Form', hideOnDevice: true, description: 'This sample enables the design of a PDF form that accommodates signatures from two different users. The form includes distinct fields for each user: when the first user is selected, specific fields can be added that apply only to that user. Upon switching the user via the dropdown menu, new fields can be added for the second user. The fields for each user are distinguishable by different background colors.'},
    // tslint:disable-next-line:max-line-length
    { path: ':theme/pdfviewer/esign-pdf-forms', component: ESigningPdfFormsComponent , name: 'eSigning PDF Forms', order: '12', type: 'update', category: 'PDF Form', description: 'This sample enables the design of a PDF form that accommodates signatures from two different users. The form includes distinct fields for each user: when the first user is selected, specific fields can be added that apply only to that user. Upon switching the user via the dropdown menu, new fields can be added for the second user. The fields for each user are distinguishable by different background colors.'},
    // tslint:disable-next-line:max-line-length
    ...
    // tslint:disable-next-line:max-line-length
    { path: ':theme/pdfviewer/invisible-signature', component: InvisibleSignatureComponent , name: 'Invisible Signature', order: '14', category: 'Signature', description: 'This sample demonstrates how to digitally sign a PDF document from code behind using Syncfusion<sup>®</sup> PDF Viewer and PDF Library'},
];
